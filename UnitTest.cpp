...
#include "D:/pybind11/pybind11-master/include/pybind11/pybind11.h"
#include "D:/pybind11/pybind11-master/include/pybind11/embed.h"
#include "UnitTest.h"
#include <Python.h>
#include <iostream>
#include <vector>
...
//下面是新增代码
void TestDeepLearningIntegration() {
	std::vector<float> input_data = { 1.0, 2.0, 3.0, 4.0 }; // 示例输入数据
	std::vector<float> result = call_python_predict(input_data); // 调用Python的预测函数
...
int main() {
	...
	TestDeepLearningIntegration();
	return 0;
}



//上面这个代码是之前可以跑的，要是新添加的下面的没法跑，那就删除
